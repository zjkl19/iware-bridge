...
$(function() {
  ...
  var compose = arry[0].split("=")[1];
  var parNam = arry[1].split("=")[1];

  console.log(compose, parNam);

  var szIP = null;
  var szPort = null;
  var szUsername = null;
  var szPassword = null;
  // var szIP = "172.16.5.204",
  // szPort = "80",
  // szUsername = "admin",
  // szPassword = "anheng@123";
  // 500 * 300
  var videoWidth = 1100;
  var videoHeight = 660;

  if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
    console.log("您还未安装过插件，请确认下载并安装WebComponents.exe！");
    if (parNam && parNam == "true") {
      if (
        confirm(
          "您还未安装过视频插件，点击确定下载并安装WebComponents.exe！并且使用360浏览器打开！"
        ) == true
      ) {
        window.location.href = "/static/WebComponents.exe";
        setTimeout(() => {
          window.location.href =
            "http://down.360safe.com/se/360se10.1.2245.0.exe";
        }, 1000);
      }
    }
    // var $eleForm = $("<form method='get'></form>");

    // $eleForm.attr("action","/static/WebComponents.exe");
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      for (let i = 0; i < res.data.length; i++) {
        ...
        szIP = element.videoIp;
        szPort = element.videoPort;
        szUsername = element.videoName;
        szPassword = element.videoPassword;
        // if (element.y == 9) {
        // 	//九宫格，视频大小为382*240
        // 	// videoWidth = "75%";
        // 	// videoHeight = "80%";
        ...
        // 	videoHeight = 755;
        // }
        videoWidth = $("#demoidid").width();
        videoHeight = $("#demoidid").height();
        // console.log("%%%%%%%%%%%%%",$("#demoidid").width(),$("#demoidid").height())
        if (parNam && parNam == "true") {
          //表示从首页详情跳转过来的
          // videoWidth = 300;
          // videoHeight = 180;
