<template>
  <div
    :id="'analysisLineArea' + echartData.id"
    class="analysisLineArea"
    style="width: 100%; height: 100%"
  ></div>
</template>

<script>
import erd from 'element-resize-detector';
export default {
  name: 'analysisLineArea-dzl',
  components: {},
  props: {
    typeIndex: {
      type: Number
    },
    echartData: {
      type: Object,
      default: () => {}
    }
  },
  data() {
    return {
      myChart: '',
      size: window.innerHeight / 70 / 7
    };
  },
  methods: {
    setOption() {
      let _this = this;
      this.myChart.clear();
      this.myChart.setOption(
        {
          tooltip: {
            trigger: 'axis',
            confine: true,
            backgroundColor: 'rgba(0, 0, 0, 0.5)',
            formatter: function (params) {
              // console.log(params);
              let html = `<div class="analysisToolBox">`;
              html += `<div>${params[0].name}</div>`;
              html += `<div>${params[0].seriesName}：${params[0].value}</div>`;
              html += `</div>`;
              return html;
            }
          },
          grid: {
            top: '15%', //与上面的距离
            left: '3%', //与左边的距离
            right: '4%', //与右边的距离
            bottom: '3%', //与下面的距离
            containLabel: true
          },
          xAxis: {
            type: 'category',
            boundaryGap: false,
            triggerEvent: true,
            data: this.echartData.xData,
            axisLabel: { color: '#595959', fontSize: 6 * this.size },
            axisLine: { lineStyle: { color: '#BFBFBF' } },
            axisTick: { lineStyle: { color: '#BFBFBF' } },
            splitArea: { show: false }
          },
          yAxis: {
            type: 'value',
            max: () => {
              let dataList = _this.echartData.yData;
              let dataArry = [];
              dataList.map((item) => {
                if (item != null) {
                  if (item.value) {
                    dataArry.push(item.value);
                  } else {
                    dataArry.push(item);
                  }
                }
              });

              let maxValue = Math.max(...dataArry);
              maxValue = Math.ceil(maxValue * 100) / 100;
              return maxValue;
            },
            min: () => {
              let dataList = _this.echartData.yData;
              let dataArry = [];
              dataList.map((item) => {
                if (item != null) {
                  if (item.value) {
                    dataArry.push(item.value);
                  } else {
                    dataArry.push(item);
                  }
                }
              });
              let minValue = Math.min(...dataArry);
              minValue = Math.floor(minValue * 100) / 100;
              return minValue;
            },
            triggerEvent: true,
            name: this.echartData.unit,
            nameTextStyle: { color: '#595959' },
            axisTick: { show: false },
            axisLine: { lineStyle: { color: '#BFBFBF' } },
            axisLabel: {
              color: '#595959',
              fontSize: 7 * this.size
              // formatter: function (val, index) {
              //   let str = JSON.stringify(val * 100);
              //   let arry = str.split('.');
              //   let value = Number(arry[0]) / 100;
              //   return value;
              // }
            },
            splitArea: { show: false },
            splitLine: { lineStyle: { color: '#DEDEDE', type: 'dashed' } }
          },
          series: [
            {
              name: this.echartData.name,
              type: 'line',
              smooth: false,
              symbolSize: 1,
              // symbol: 'none',
              itemStyle: {
                color: '#4EA7F9'
              },
              areaStyle: {
                color: new this.$echarts.graphic.LinearGradient(0, 0, 0, 1, [
                  {
                    offset: 0,
                    color: 'rgba(78, 166, 249, 0.2)'
                  },
                  {
                    offset: 1,
                    color: 'transparent'
                  }
                ])
              },
              data: this.echartData.yData
            }
          ]
        },
        false
      );
    }
  },
  mounted() {
    let _this = this;
    let id = 'analysisLineArea' + this.echartData.id;
    this.myChart = this.$echarts.init(document.getElementById(id));
    this.setOption();
    window.addEventListener('resize', () => {
      this.myChart.resize();
    });
    this.myChart.on('click', function (params) {
      // console.log(params);
      if (params.componentType === 'xAxis') {
        _this.$emit('getFrequencyPoint', params.value, _this.typeIndex);
      }
      if (params.componentType === 'series') {
        _this.$emit('getFrequencyPoint', params.name, _this.typeIndex);
      }
    });
    erd().listenTo(document.getElementById(id), () => {
      //监听到事件后执行的业务逻辑
      _this.myChart.resize();
    });
  },
  beforeDestroy() {
    if (!this.myChart) {
      return;
    }
    window.removeEventListener('resize', {});
  }
};
</script>
<style lang="scss">
.analysisToolBox {
  padding: 4px;
  border-radius: 2px;
  div {
    font-size: 12px;
    line-height: 17px;
  }
}
</style>
