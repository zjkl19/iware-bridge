  ...
  data() {
    return {
      ...
      headers: {
        ...
        'X-ROUTER-URL': '/infoManage/tunnelManage'
      },
      actionUrl: '',
  ...
  methods: {
    ...
    async getList() {
      ...
      if (this.structureId != null) {
        let { code, data } = await listByPage(pageInfo, this.structureId, 4);
        if (code == '0000') {
    ...
    showAddAnnexModel(type) {
      if (type == 1) {
        this.title = '上传附件';
        this.actionUrl = '/bridge/photo/model/upload/4';
    ...
    async submitAnnexUpload() {
      ...
      if (this.title == '上传附件') {
        let { code } = await uploadModel(this.formData, 4);
        if (code == '0000') {
          this.$message({
            message: '上传成功',
            showClose: true,
